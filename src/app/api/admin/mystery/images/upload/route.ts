// src/app/api/admin/mystery/images/upload/route.ts
...
export async function POST(request: Request) {
  try {
    ...
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'mystery');
    ...
    const publicUrl = `/uploads/mystery/${uniqueName}`;

    return NextResponse.json({ success: true, data: { url: publicUrl } });
  } catch (error) {
    console.error('Mystery image upload error:', error);
